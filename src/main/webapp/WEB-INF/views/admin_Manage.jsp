	...
	<div class="bk-con-message message-section" id="iframe_box">
           ...
           <div class="operation mb15">
			   <select class="form-control "style="width: 80px; display:inline" id="sel">
				   <option value="ad_id">管理员编号</option>
				   <option value="ad_type">管理类型编号</option>
				   <option value="ad_phone">电话</option>
			    ...
			    <button class="btn button_btn btn-danger" type="button" onclick="addAdmin()"><i class="iconfont"></i>&nbsp;添加管理员</button>
...
function findUserInfo(){
	...
	postAjax("selectAdminInfoByParam",null,
		{
		    "param":sel,
			"value":txtContent
		},
...
function addAdmin(){
    window.location.href="/jumpAddAdminInfo";
}
...
function forEachData(data){
    ...
    for (var i=0; i< data.length ; i++){
        ...
        var $tr = $("<tr class=\"list-users\">"+
            "<td>"+data[i].adId+"</td>"+
            "<td>"+data[i].adType+"</td>"+
            "<td>"+data[i].adPhone+"</td>"+
            "<td>"+data[i].adPassword+"</td>"+
            "<td>"+date+"</td>"+
            "<td>"+data[i].adName+"</td>"+
            "<td><button type='button' class='btn button_btn bg-deep-gray' onclick='clickDeleteUserInfo(\""+ data[i].adId + "\")' ><i class=\"iconfont\">删除</i></button>"+
            "<button type='button'  class='btn button_btn bg-deep-gray'   onclick='clickUpdateUserInfo(\""+ data[i].adId + "\")'> <i class=\"iconfont\">修改</i></button>"+
            "</tr>");
...
function clickUpdateUserInfo(data){
    console.log(data);
	window.location.href="/jumpEditAdminInfo?adId="+data;
...
function clickDeleteUserInfo(data){
    console.log(data);
    postAjax("/deleteAdminInfoById",null,{
        adId:data
	},function(data){
        alert("删除成功！");
		forEachData(data);
